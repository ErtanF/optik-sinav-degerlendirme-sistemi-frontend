...
const BubbleGrid = memo(function BubbleGrid({ rows, cols, characters, type }) {
  // Kodlanabilir daireler için grid oluştur
  
  // Dikey düzende display
  const isVertical = type === 'nameSurname' || type === 'number';
      ...
      <div className={styles.horizontalGrid}>
        ...
        {Array.from({ length: rows }).map((_, rowIndex) => (
          <div key={`question-${rowIndex}`} className={styles.questionRow}>
            <div className={styles.questionNumber}>{rowIndex + 1}</div>
            <div className={styles.choices}>
              {characters.slice(0, Math.min(cols, characters.length)).map((char, colIndex) => (
                ...
                >
                  <div className={styles.bubble} title={`Soru ${rowIndex + 1}, Şık ${char}`}>
  ...
  return isVertical ? renderVerticalGrid() : renderHorizontalGrid();
});
